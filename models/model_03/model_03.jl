...
function sample_batch(fluids, dt)
    ...
    u = rand() * 1.2
    v = rand() * 1.2
    x_5, y1_5, y2_5 = (cat(u * ones(65, 64, 2), v * ones(65, 64, 2), dims=3),
                       cat(u * ones(129, 128, 2), v * ones(129, 128, 2), dims=3),
                       cat(u * ones(257, 256, 2), v * ones(257, 256, 2), dims=3))
    x_6, y1_6, y2_6 = (zeros(65, 64, 4), zeros(129, 128, 4), zeros(257, 256, 4))
    # Compute increments
